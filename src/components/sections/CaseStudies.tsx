...
export default function CaseStudies() {
  return (
    <section className="max-w-6xl mx-auto px-6 py-12">
      <h2 className="text-3xl font-bold text-blue-800 mb-8 text-center">Client Success Stories</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {cases.map((cs) => (
          <div key={cs.title} className="bg-white border rounded-xl p-7 shadow flex flex-col items-center text-center">
            {cs.icon}
            <h3 className="font-semibold text-lg mb-2 text-blue-700">{cs.title}</h3>
            <p className="text-gray-600 text-sm">{cs.body}</p>
          </div>
        ))}
      </div>
    </section>
